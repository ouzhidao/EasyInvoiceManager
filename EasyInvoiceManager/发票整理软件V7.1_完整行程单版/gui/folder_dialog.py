...
class FolderDialog(QDialog):
    ...
    def _get_drives_fast(self):
        """使用Windows API快速获取磁盘列表，避免网络驱动器超时"""
        drives = []
        try:
            import ctypes
            buf = ctypes.create_string_buffer(260)
            kernel32 = ctypes.windll.kernel32
            result = kernel32.GetLogicalDriveStringsA(260, buf)
            if result:
                drives_str = buf.raw.decode('ascii').rstrip('\x00')
    ...
    def update_tree(self, current_path: str):
        ...
        if os.name == 'nt':
            ...
            # 【修复】使用Windows API快速获取磁盘列表，避免网络驱动器超时
            drives = self._get_drives_fast()
            for drive in drives:
    ...
    def show_computer(self):
        """显示此电脑 - 使用快速API避免网络驱动器超时"""
        self.tree_folders.clear()
        
        # 【修复】使用Windows API快速获取磁盘列表
        drives = self._get_drives_fast()
        for drive in drives:
            letter = drive[0].upper()
            item = QTreeWidgetItem([f"{letter}:"])
